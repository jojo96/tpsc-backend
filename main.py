...
app.add_middleware(
    ...
    allow_origins=[
        "http://localhost:5173",
        "https://tpsc-frontend.vercel.app"
    ],
    allow_methods=["*"],
    allow_headers=["*"],
)
...
def get_rag_chain():
    from rag import get_rag_chain as _get_rag_chain
...
async def daily_trivia():
    ...
    llm = get_llm()
    prompt = f"""Generate exactly 5 current affairs trivia questions for TPSC exam preparation.
...
- Make questions useful for a state PCS exam
